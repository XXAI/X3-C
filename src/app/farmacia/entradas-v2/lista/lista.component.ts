...
export class ListaComponent implements OnInit {
  ...
  private resultadosBusqueda: Entrada[] = [];
  ...
  ngOnInit() {
    ...
    .switchMap((term:string)  =>  { 
      ...
      this.cargando = true;
      return term  ? this.entradasService.buscar(term, this.paginaActualBusqueda, this.resultadosPorPaginaBusqueda) : Observable.of<any>({data:[]}) 
    }
    ...
    busquedaSubject.subscribe(
      resultado => {
        this.cargando = false;
        this.resultadosBusqueda = resultado.data as Entrada[];
  ...
  listarBusqueda(term:string ,pagina:number): void {
    ...
    this.cargando = true;
    this.entradasService.buscar(term, pagina, this.resultadosPorPaginaBusqueda).subscribe(
        resultado => {
          this.cargando = false;
  ...
  listar(pagina:number): void {
    ...
    this.cargando = true;
    this.entradasService.lista(pagina,this.resultadosPorPagina).subscribe(
        resultado => {
          this.cargando = false;
          this.entradas = resultado.data as Entrada[];
  ...
  eliminar(entrada: Entrada, index): void {
    entrada.cargando = true;
    this.entradasService.eliminar(entrada.id).subscribe(
        data => {
          ...
          this.mensajeExito.texto = "Entrada eliminado";
        },
        error => {
          entrada.cargando = false;
          this.mensajeError.mostrar = true;
