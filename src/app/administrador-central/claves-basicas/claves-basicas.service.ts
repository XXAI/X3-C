...
export class ClavesBasicasService {
    ...
    listaClues(id:any,pagina:number = 1, resultados_por_pagina:number =20 ): Observable<any>{
        return this.jwtRequest.get("administrador-central/claves-basicas-clues",id,{page: pagina, per_page: resultados_por_pagina}).map( (response: Response) => response.json().data);
    }
}
