...
export class NuevoComponent implements OnInit {
  ...
  constructor(private title: Title,
    ...
    private route: ActivatedRoute,private apiService: PresupuestoService,  private http: Http) { }
  ...
  guardar(){
    ...
    if(this.errores.presupuesto_unidades_medicas !=  null){
      for(var i in this.presupuesto.presupuesto_unidades_medicas){
        ...
        if(this.presupuesto.presupuesto_unidades_medicas[i].error_validacion != null){
          this.presupuesto.presupuesto_unidades_medicas[i].error_validacion = null;
        }
      } 
    }
    ...
    this.apiService.crear(this.presupuesto).subscribe(
      respuesta => {
            ...
            case 409:
              ...
              for (var input in e.error) {
                ...
                for (var i in e.error[input]) {
                  ...
                  if(object.length > 1){
                    if(object[0] == "presupuesto_unidades_medicas"){
                      ...
                      if(object[2] == 'causes'){
                        //this.presupuesto.presupuesto_unidades_medicas.
                        this.presupuesto.presupuesto_unidades_medicas[index].error_validacion = {};
                        this.presupuesto.presupuesto_unidades_medicas[index].error_validacion.causes = e.error[input][i];
                      }

                      if(object[2] == 'no_causes'){
                        if(this.presupuesto.presupuesto_unidades_medicas[index].error_validacion == null){
                          this.presupuesto.presupuesto_unidades_medicas[index].error_validacion = {};
                        }
                        this.presupuesto.presupuesto_unidades_medicas[index].error_validacion.no_causes = e.error[input][i];
                      }
              ...
              console.log(this.presupuesto.presupuesto_unidades_medicas);
              break;
            case 500:
    ...
    this.apiService.cargarUltimoPresupuesto().subscribe(
      respuesta =>{
        if(respuesta){
          ...
          this.presupuesto.presupuesto_unidades_medicas = respuesta.presupuesto_unidades_medicas;
          for(var i in  this.presupuesto.presupuesto_unidades_medicas){
            this.cluesAgregadas.push( this.presupuesto.presupuesto_unidades_medicas[i].clues);
          }
  ...
  agregarUnidadMedica(items:any[]){
    for(var i in items){
      ...
      this.presupuesto.presupuesto_unidades_medicas.push(object);
    }
  }
  ...
  eliminarUnidadMedica(item:any, index){
    ...
    this.presupuesto.presupuesto_unidades_medicas.splice(index, 1);  
    this.calcularTotales();
  }

  factorMes():string{

    switch(Number(this.presupuesto.factor_meses)){
  ...
  calcularTotales(){
    ...
    for(var i in this.presupuesto.presupuesto_unidades_medicas){
      if(this.presupuesto.presupuesto_unidades_medicas[i].error){
        con_errores = true;
        continue;
      }
      if(!isNaN(this.presupuesto.presupuesto_unidades_medicas[i].causes)){
        this.causes_sumado += Number(this.presupuesto.presupuesto_unidades_medicas[i].causes);
      } else {
        this.presupuesto.presupuesto_unidades_medicas[i].causes = 0;
      }

      if(!isNaN(this.presupuesto.presupuesto_unidades_medicas[i].no_causes)){
        this.no_causes_sumado += Number(this.presupuesto.presupuesto_unidades_medicas[i].no_causes);
      } else {
        this.presupuesto.presupuesto_unidades_medicas[i].no_causes = 0;
      }
    }   
  ...
  subir() {
      ...
      formData.append('archivo', this.archivo, this.archivo.name);
            ...
            this.presupuesto.causes = data.causes;
            this.presupuesto.no_causes = data.no_causes;
            ...
            this.causes_sumado = 0;
            this.no_causes_sumado = 0;
            for(var i in data.presupuesto_unidades_medicas){
              if(data.presupuesto_unidades_medicas[i].error){
                con_errores.push(data.presupuesto_unidades_medicas[i]);
              } else{
                correctos.push(data.presupuesto_unidades_medicas[i]);
                this.causes_sumado += Number(data.presupuesto_unidades_medicas[i].causes);
                this.no_causes_sumado += Number(data.presupuesto_unidades_medicas[i].no_causes);
              }
            }

            this.presupuesto.presupuesto_unidades_medicas = con_errores.concat(correctos);
